...
class SiteConf:
    ...
    _SITE_LOGIN_XPATH = {
        ...
        "captcha": [
            ...
            '//input[contains(@id, "seccodeverify_")]',
        ...
        "captcha_img": [
            ...
            '//span[contains(@id, "vseccode_")]//img/@src',
            '//img[contains(@class, "rucaptcha-image")]/@src',
        ],
        "submit": [
            '//input[@type="submit"]',
            '//button[@type="submit"]',
        ...
        "error": [
            ...
            '//div[@id="ntcwin"]//div[contains(@class, "pc_inner")]/text()',
            '//div[contains(@class, "message-header")]/text()',
        ],
        "twostep": [
    ...
    def get_checkin_conf(self):
        internal_conf = list(self._SITE_CHECKIN_XPATH)
        custom_conf = os.path.join(Config().get_config_path(), "siteconf/SITE_CHECKIN_XPATH.txt")
        if os.path.exists(custom_conf):
            try:
                ...
                log.error(f"【User】读取 SITE_CHECKIN_XPATH 出错：{e}")
    ...
    def get_login_conf(self):
        internal_conf = dict(self._SITE_LOGIN_XPATH)
        custom_conf = os.path.join(Config().get_config_path(), "siteconf/SITE_LOGIN_XPATH.json")
        if os.path.exists(custom_conf):
            try:
                with open(custom_conf, 'r') as conf:
                    json_conf = conf.read()
                conf_dict = json.loads(json_conf)
                internal_conf.update(conf_dict)
            except Exception as e:
                ExceptionUtils.exception_traceback(e)
                log.error(f"【User】读取 SITE_LOGIN_XPATH 出错：{e}")
        return internal_conf
